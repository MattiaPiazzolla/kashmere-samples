...
export default function SampleRow({
    ...
    return (
        <tr className={`border-b border-zinc-800 hover:bg-zinc-800/40 transition-colors ${isThisTrack ? "bg-zinc-800/30" : ""}`}>
            ...
            {/* BPM / Key */}
            <td className="px-4 py-3 text-sm text-zinc-400">{sample.bpm ?? "—"} / {sample.key ?? "—"}</td>
            ...
            <td className="px-4 py-3">
                {!deleted && (
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${sample.is_published ? "bg-green-500/20 text-green-400" : "bg-zinc-700 text-zinc-400"
                        }`}>
            ...
            <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                    {!deleted ? (
                        <>
                            <button onClick={togglePublish} disabled={loading} title={sample.is_published ? "Unpublish" : "Publish"}
                            ...
                            <button onClick={() => onEdit(sample)} title="Edit"
                                className="rounded p-1.5 text-zinc-400 hover:bg-zinc-700 hover:text-white transition-colors">
                                <Pencil size={14} />
                            </button>
                            <button onClick={softDelete} disabled={loading} title="Delete"
                                className="rounded p-1.5 text-zinc-400 hover:bg-red-500/20 hover:text-red-400 transition-colors">
                                <Trash2 size={14} />
                            </button>
                        </>
                    ) : (
                        <button onClick={restore} disabled={loading} title="Restore"
                            className="rounded p-1.5 text-zinc-400 hover:bg-zinc-700 hover:text-white transition-colors">
                            <RotateCcw size={14} />
                        </button>
                    )}
